            ...
            if(isset($this->value_entries["$name"])) {
                ...
                if($type=="tooltip") {
                    ...
                    if (isset($this->value_entries["$name"]['infos']['parent_obj'])
                        ...
                        // Search parent object header robustly (spaces differ between files).
                        $parent_obj_line = false;
                        $parent_obj_pattern = '/^' . preg_quote((string)$parent_obj, '/') . '\s+0\s+obj\b/';
                        foreach ($this->pdf_entries as $line_index => $line_content) {
                            if (preg_match($parent_obj_pattern, trim($line_content))) {
                                $parent_obj_line = $line_index;
                                break;
                            }
                        }
                        if ($parent_obj_line === false) {
                            ...
                            for ($line_index = $parent_obj_line + 1, $line_count = count($this->pdf_entries); $line_index < $line_count; $line_index++) {
                                $line_content = $this->pdf_entries[$line_index];
                                $trimmed_line_content = trim($line_content);
                                if (preg_match('/^endobj\b/', $trimmed_line_content)) {
                                    break;
                                }

                                if (preg_match('/\/V\s\/[^\s<>\[\]\(\)]+/', $line_content)) {
                                    $updated_line = preg_replace('/\/V\s\/[^\s<>\[\]\(\)]+/', '/V /' . $state, $line_content, 1);
                                    if (is_string($updated_line)) {
                                        $offset_shift = strlen($updated_line) - strlen($line_content);
                                        $this->pdf_entries[$line_index] = $updated_line;
                                        if ($offset_shift !== 0) {
                                            $this->apply_offset_shift_from_object($parent_obj, $offset_shift);
                                        }
                                        $parent_value_applied = true;
                                        break;
                                    }
                                }

                                if (preg_match('/^\/T\s?\(.+\)\s*$/', $trimmed_line_content)) {
                                    $updated_line = rtrim($line_content) . $state_line;
                                    $offset_shift = strlen($updated_line) - strlen($line_content);
                                    $this->pdf_entries[$line_index] = $updated_line;
                                    if ($offset_shift !== 0) {
                                        $this->apply_offset_shift_from_object($parent_obj, $offset_shift);
                            ...
                            if (!$parent_value_applied && isset($this->pdf_entries[$parent_obj_line + 1])) {
                                $line_content = $this->pdf_entries[$parent_obj_line + 1];
                                $updated_line = rtrim($line_content) . $state_line;
                                $offset_shift = strlen($updated_line) - strlen($line_content);
                                $this->pdf_entries[$parent_obj_line + 1] = $updated_line;
                                if ($offset_shift !== 0) {
                                    $this->apply_offset_shift_from_object($parent_obj, $offset_shift);
                                }
                            }
...
//					$this->Error("set_field_value failed as invalid valuetype $type for object $object_id");
        ...
        function get_xref_start_line() {
            ...
            $count = count($entries);
            for ($i = 0; $i < $count; $i++) {
                if (!preg_match('/^\s*startxref\b/', $entries[$i])) {
                    continue;
                }
                for ($j = $i + 1; $j < $count; $j++) {
                    $candidate = trim($entries[$j]);
                    if ($candidate === '') {
                        continue;
                    }
            ...
            $this->Error("Unable to locate startxref value line in PDF.");
                                ...
                                if($subtype=="Widget") {
                                    ...
                                    if(($name == '') && $this->useCheckboxParser && isset($object["infos"]["parent_obj"])) {
                                        ...
                                        if(isset($object_names_by_id[$parent_obj_id]) && $object_names_by_id[$parent_obj_id] != '') {
                                            ...
                                            $name_inherited_from_parent = true;
                                        } else {
                                            if(!isset($pending_widgets_by_parent[$parent_obj_id])) {
                                                $pending_widgets_by_parent[$parent_obj_id] = array();
                                            }
                                            $pending_widgets_by_parent[$parent_obj_id][] = $object;
                                        }
                                    }
                                ...
                                if(array_key_exists("length",$stream)) { //length is mandatory
                                    ...
                                    */
                                    ...
                                    if($this->useCheckboxParser && ('' == $ap_d_yes || '' == $ap_d_no || '' == $as)) {
                                        ...
                                        if (($ap_d_yes=='' || $ap_d_no=='') && preg_match('/\/D\s*<<\s*\/(\w+)\s+\d+\s+\d+\s+R\s*\/(\w+)\s+\d+\s+\d+\s+R/', $CurLine, $direct_match)) {
                                            $candidate_one = $direct_match[1];
                                            $candidate_two = $direct_match[2];
                                            if (strtolower($candidate_one) == 'off') {
                                                $ap_d_no = $candidate_one;
                                                $ap_d_yes = $candidate_two;
                                            } elseif (strtolower($candidate_two) == 'off') {
                                                $ap_d_yes = $candidate_one;
                                                $ap_d_no = $candidate_two;
                                            } else {
                                                $ap_d_yes = $candidate_one;
                                                $ap_d_no = $candidate_two;
                                            }
                                        ...
                                        if (($ap_line==$Counter-5)&&($ap_d_line==$Counter-3)&&($ap_d_no=='')&&$this->extract_pdf_definition_value("name", $CurLine, $match)) {
                                            $ap_d_no=$match[1];
                                            if ($verbose_parsing) {
                                        ...
                                        if (($as=='')&&$this->extract_pdf_definition_value("/AS", $CurLine, $match)) {
                                            $as=$match[1];
                                            if ($verbose_parsing) {
                            ...
                            if($refs_count>0) {
                                ...
                                if($trailer_table==1) { //should be <<
                                    ...
                                    switch($trailer_table) {
                                        case -1://startxref
                                            if(!preg_match("/^\s*startxref/",$CurLine,$match))
                                                $this->Error("startxref tag expected, read $CurLine");
                                            break;
                                        case -2://startxref's value
                                            if(preg_match("/^\s*(\d+)/",$CurLine,$match)) {
